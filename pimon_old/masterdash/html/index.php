...
$result = mysqli_query($con,"SELECT * FROM registry"); //WHERE `checkin_time` BETWEEN '$date_fiveminago' AND '$date_now' GROUP BY ip");
//$result = mysqli_query($con,"SELECT * FROM registry WHERE `checkin_time` BETWEEN '$date_fiveminago' AND '$date_now' GROUP BY ip");
...
while($row = mysqli_fetch_array($result)) {
  ...
  echo "</td>";
  echo "<td>";
  echo "<a href=http://madasilva0d:8080/del-url.php?url={$row['url']}> Delete </a>";
  echo "</td>";
  echo "</tr>";
}
echo "</table>";
...
while($row_registry = mysqli_fetch_array($result))
{
  ...
  while($row = mysqli_fetch_array($result2)) {
      ...
      echo "</td>";
      echo "<td>";
      echo "<a href=http://madasilva0d:8080/del-each-url.php?url={$row['url']}&ip=$ip&time={$row['time']}> Delete </a>";
      echo "</td>";
      echo "</tr>";
  }
  echo "</table>";
